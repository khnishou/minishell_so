...
/*   Updated: 2023/08/20 22:53:43 by ykerdel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_split(char const *s, char c, t_data *g)
{
	char	**t;
	int		j;
	int		l;
	...
	j = 0;
	l = 0;
	t = (char **)ft_malloc(&g->mem_list, (ft_wc(s, c) + 1) * sizeof(char *));
	if (!t)
		return (NULL);
	while (s[l])
	{
		...
		{
			t[j] = (char *)ft_malloc(&g->mem_list, (ft_split_len(s, l, c) + 1));
			if (!t[j])
