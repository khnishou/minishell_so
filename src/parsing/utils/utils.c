...
/*   Updated: 2023/08/17 00:30:15 by ykerdel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ms_swapstr(char *src, char *swap, size_t *ints, t_data *g_data)
{
    ...
    if (!src || ints[_start] >= src_len)
        return (NULL);
    end = ints[_start] + ints[_len];
    res = ft_malloc(&(g_data->mem_list), src_len - ints[_len] + ft_strlen(swap) + 1);
    if (!res)
        return (NULL);
    i = -1;
    while (++i < (int) ints[_start])
        res[i] = src[i];
    ints[_start] = 0;
    if (swap)
        while (swap[ints[_start]])
            res[i++] = swap[ints[_start]++];
    while (src[end])
        res[i++] = src[end++];
    res[i] = '\0';
    return (res);
